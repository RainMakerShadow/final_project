        ...
        <div class="flex justify-between px-1 sm:px-4 mx-auto max-w-screen-xl ">
            <article class="mx-auto w-full max-w-5xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
                <header class="mb-4 lg:mb-6 not-format">
                    <h1 class="mb-4 text-2xl font-extrabold text-center leading-tight text-gray-900 lg:mb-6 lg:text-4xl dark:text-white">{{$news[0]->title}}</h1>
                </header>
                <p class=" text-sm sm:text-2xl lead dark:text-white text-black">{{$news[0]->content}}</p>
                <figure class="mt-10 text-center m-auto"><img class="text-center m-auto" src="{{ Illuminate\Support\Facades\Storage::url('image/news/'.$news[0]->img) }}" alt="{{$news[0]->img_alt}}">
                    <figcaption class="text-sm sm:text-2xl lead dark:text-white text-black">{{$news[0]->img_descr}}</figcaption>
                </figure>
            </article>
        </div>
    </main>
            ...
            <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
                @foreach($news_list as $news)
                    <article class="max-w-xs">
                        <a href="{{$news->link}}">
                            <img src="{{ Illuminate\Support\Facades\Storage::url('image/articles/'.$news->img) }}" class="mb-5 rounded-lg" alt="Image 1">
                        </a>
