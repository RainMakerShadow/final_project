                            ...
                            @foreach($count as $item)
                                @if($item->product_id === $product->id )
                                    <tr class="border-b dark:border-gray-700" id="{{$product->id}}">
                                        ...
                                        <td class="px-4 py-3">
                                                    <input wire:click.prevent="inputClick($event.target.id, $event.target.value)" id="{{$product->id}}" name="{{$product->id}}" type="number" value="{{$item->quantity}}" class="w-16 h-10">
                                        </td>
                                        <td class="px-4 py-3">₴
                                            @if($product->discount)
                                        ...
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            <button wire:click.prevent="deleteItem($event.target.dataset.id)" id="delete-{{$product->id}}" data-id="{{$item->id}}" data-dropdown-toggle="apple-imac-27-dropdown" class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                                Видалити
                                            </button>
                                        </td>
                                    </tr>
