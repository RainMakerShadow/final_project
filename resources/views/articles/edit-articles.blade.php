<div>
    ...
    <div class="p-4 pb-10">
        <h1 class="mb-4  font-extrabold text-gray-500 dark:text-white md:text-2xl lg:text-3xl">Редагування статті<span class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400"> @if($title) "{{$title}}" @endif</span> </h1>
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    ...
                    <select wire:model="category" id="selected" name="selected" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach($category_selected as $articles_categories_item)
                            <option value="{{$articles_categories_item->id}}" >{{$articles_categories_item->title}}</option>
                        @endforeach
                    </select>
                    {{--                    --}}
