<div>
    <div class="p-4 pb-10">
        <h1 class="mb-4  font-extrabold text-gray-500 dark:text-white md:text-2xl lg:text-3xl">Редагування категорії<span class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400"> "{{$title}}"</span> </h1>
        <form wire:submit.prevent="submit" enctype="multipart/form-data" class="shadow-xl">
                ...
                <div>
                    ...
                    <select wire:model="selected" id="selected" name="selected" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach($menu as $menu_item)
                            <option value="{{$menu_item->id}}" @if($menu_item->id===$article_category->menus_id) selected @else @endif>{{$menu_item->title}}</option>
                        @endforeach
                    </select>
                    {{--                    --}}
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div class="grid justify-items-center">
                    @if (is_object($image))
                        <img class="h-auto max-w-xl rounded-lg shadow-lg shadow-gray-700 dark:shadow-cyan-500/50" src= "{{$image->temporaryUrl()}}"  alt="image description" width="50%">
                    @else
                        <img class="h-auto max-w-xl rounded-lg shadow-lg shadow-gray-700 dark:shadow-cyan-500/50" src= "{{$imageUrl}}"  alt="image description" width="50%">
                    @endif
                </div>
            </div>
