            ...
            <div>
                ...
                <div id="dropdownAction" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownActionButton">
                        <li>
                            <a href="{{route('news-category.add')}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white hover:rounded">Додати категорію</a>
                        </li>
                    </ul>
                    <div class="py-1">
                        <a href="{{route('news-categories.delete', [implode(',', $toDelete)])}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100  hover:rounded dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Видалити категорію</a>
                    </div>
                </div>
            </div>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 hidden lg:table-cell lg:px-6 lg:py-4">
                            @foreach($menu as $menu_item)
                                @if($menu_item->id==$news_category->menus_id)
                                    {{$menu_item->title}}
                                @endif
                            @endforeach
                        ...
                        <td class="px-6 py-4 hidden xl:table-cell lg:px-6 lg:py-4">
                            <a data-toggle="modal" data-target="#imageModal">
                                <img class="h-20 max-w-lg rounded-lg hover:scale-125 transition-transform duration-300" src="{{ Illuminate\Support\Facades\Storage::url('image/products-categories/'.$news_category->img) }}" alt="{{$news_category->img_descr}}">
                            </a>
                        </td>
